...
class SchoolMaterialBase(models.Model):
    school = models.ForeignKey(School, on_delete=models.CASCADE, related_name='material_base', verbose_name='Maktab',
                               db_index=True)
    material = models.ForeignKey(MaterialBase, on_delete=models.CASCADE, related_name='school_material_base',
                                 verbose_name='Baza', db_index=True)
...
class ClassroomEquipment(models.Model):
    school = models.ForeignKey(School, on_delete=models.CASCADE, related_name="school",
                               verbose_name='Maktab', )
    equipment = models.ForeignKey(Equipment, on_delete=models.CASCADE, verbose_name='Maktab jihozlari', related_name='equipment')
    actual_quantity = models.PositiveIntegerField(default=0, verbose_name="Amalda mavjud")
    ...
    def __str__(self):
        return f"{self.school.name} - {self.equipment.name}: {self.actual_quantity} dona"
