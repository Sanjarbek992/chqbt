...
from .serializers import (
    ...
    LessonScheduleSerializer, LessonMaterialSerializer
)
...
class LessonViewSet(viewsets.ModelViewSet):
    queryset = Lesson.objects.select_related('school').prefetch_related('schedule', 'materials').all()
...
class LessonMaterialViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['post'], url_path='upload', permission_classes=[IsAdminOrSuperAdmin])
    def upload_material(self, request):
        lesson_id = request.data.get('lesson_id')
        file = request.FILES.get('file')

        if not lesson_id or not file:
            return Response({'detail': 'lesson_id va file majburiy.'}, status=status.HTTP_400_BAD_REQUEST)
